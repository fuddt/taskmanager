...
def parse_depends(depends_text):
...
def read_tasks_from_excel(file_path):
    ...
    for row_idx in range(DATA_START_ROW, ws.max_row + 1):
        ...
        task = TaskRow(
            ...
            task_id=normalize_str(
                ws.cell(row=row_idx, column=header_map["ID"]).value
            ),

            function_name=normalize_str(
                ws.cell(row=row_idx, column=header_map["機能"]).value
            ),

            task_category=normalize_str(
                ws.cell(row=row_idx, column=header_map["タスク区分"]).value
            ),

            task_name=normalize_str(
                ws.cell(row=row_idx, column=header_map["タスク名"]).value
            ),

            display_name=normalize_str(
                ws.cell(row=row_idx, column=header_map["表示名"]).value
            ),

            owner=normalize_str(
                ws.cell(row=row_idx, column=header_map["担当"]).value
            ),

            start_date=parse_date(
                ws.cell(row=row_idx, column=header_map["開始日"]).value
            ),

            end_date=parse_date(
                ws.cell(row=row_idx, column=header_map["期限"]).value
            ),

            progress=parse_int(
                ws.cell(row=row_idx, column=header_map["進捗率"]).value
            ),

            status=normalize_str(
                ws.cell(row=row_idx, column=header_map["ステータス"]).value
            ),

            priority=normalize_str(
                ws.cell(row=row_idx, column=header_map["優先度"]).value
            ),

            display_order=parse_int(
                ws.cell(row=row_idx, column=header_map["表示順"]).value
            ),

            depends_on=parse_depends(
                normalize_str(
                    ws.cell(
                        row=row_idx,
                        column=header_map["依存タスクID"]
                    ).value
                )
            ),

            task_type=normalize_str(
                ws.cell(row=row_idx, column=header_map["タスク種別"]).value
            ).lower(),

            delay=normalize_str(
                ws.cell(row=row_idx, column=header_map["遅延"]).value
            ),

            concern=normalize_str(
                ws.cell(row=row_idx, column=header_map["課題/懸念"]).value
            ),

            next_action=normalize_str(
                ws.cell(row=row_idx, column=header_map["次アクション"]).value
            ),

            updated_at=parse_date(
                ws.cell(row=row_idx, column=header_map["更新日"]).value
            ),
...
def generate_plantuml(tasks):
    ...
    label_map = {
        t.task_id: build_unique_label(t)
        for t in tasks
    }
    ...
    for task in tasks:
        ...
        if task.function_name != current_function:
            ...
            lines.append(f"-- {current_function} --")
        ...
        lines.append(
            f"[{label}] starts {task.start_date}"
        )

        lines.append(
            f"[{label}] requires {duration} days"
        )

        lines.append(
            f"[{label}] is {task.progress}% completed"
        )

        color = status_to_color(task.status)

        lines.append(
            f"[{label}] is colored in {color}"
        )
        ...
        for dep in task.depends_on:
            ...
            if dep in label_map:
                ...
                lines.append(
                    f"[{label}] starts after [{label_map[dep]}]'s end"
                )
